<template>
    <Modal ref="modalParecer" title="Parecer" modal-dialog-class="modal-xl">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            ...
                            Status: <strong>
                                <span v-if="parecer_afugentamento?.fk_status === 1" class="badge bg-yellow-lt">
                                        Em análise
                                </span>
                                <span v-else-if="parecer_afugentamento?.fk_status === 3" class="badge bg-blue-lt">
                                        Aprovado
                                    </span>
                                <span v-else-if="parecer_afugentamento?.fk_status === 2" class="badge bg-red-lt">
                                        Pendente
                                </span>
                        ...
                        <div class="mb-2" v-if="parecer_afugentamento?.fk_status === 1">
                        ...
                        <div class="mb-2" v-if="parecer_afugentamento?.fk_status !== 1">
                            ...
                            Data do parece: <strong>{{ parecer_afugentamento?.updated_at }}</strong>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <template #footer>
            <NavButton @click="emiteParecer(3)" v-if="parecer_afugentamento?.fk_status === 1" type-button="success"
                       :icon="IconCheck" title="Aprovar"/>
            <NavButton @click="emiteParecer(2)" v-if="parecer_afugentamento?.fk_status === 1" type-button="danger"
...
const parecer_afugentamento = ref(null);

const abrirModal = (item) => {
    console.log(item)
    servico.value = item;
    parecer_afugentamento.value = item.parecer_afugentamento;
    form.id = item.parecer_afugentamento.id
    form.parecer = item.parecer_afugentamento.parecer
...
const emiteParecer = (status) => {
    ...
    form.post(route('fiscal.emite-parecer-config-afugentamento'), {
