...
const modalVincularABIORet = ref({});
...
const abrirModal = () => {
  ...
  modalVincularABIORet.value.getBsModal().show();
}

const getLicenca = () => {
  axios.get(route('contratos.contratada.servicos.monitora_fauna.configuracoes.vincular_abio.get_licenca', { contrato: props.contrato.id, servico: props.servico.id, licenca: numero_licenca.value.id })).then(r => {
    licenca.value = r.data;
  });
}

const vincularABIO = () => {
  router.post(route('contratos.contratada.servicos.monitora_fauna.configuracoes.vincular_abio.store', { contrato: props.contrato.id, servico: props.servico.id }), { id_licenca: licenca.value.id }, {
    onSuccess: () => modalVincularABIORet.value.getBsModal().hide()
  });
}
...
<template>
  <Modal ref="modalVincularABIORet" title="Vincular ABIO" modal-dialog-class="modal-xl">
    <template #body>
      <div class="row">
