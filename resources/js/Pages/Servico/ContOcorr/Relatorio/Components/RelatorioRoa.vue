...
const props = defineProps({
    analise: {type: Object},
    roasAtendidos: {type: Array},
    roasEmAberto: {type: Array}
});
</script>
<template>
    <h2>10. Resultados</h2>
    <div class="mb-4">
        <h4>10.1 Tabela com os ROA's atendidos no período</h4>
        <div class="card mb-4">
            <div class="table-responsive">
                <table class="table card-table table-bordered">
                    ...
                    <tbody>
                    <tr v-for="roa in roasAtendidos" :key="roa.id">
                        <td>{{ roa.nome_id }}</td>
        ...
        <div class="row">
            <div class="col">
                <span><strong>Análise: </strong>{{ analise?.roas_atendidos }}</span>
            </div>
        </div>
    </div>
    <div>
        <h4>10.2 Tabela com os ROA's em aberto no período</h4>
        <div class="card mb-4">
            <div class="table-responsive">
                <table class="table card-table table-bordered">
                    <thead>
                    ...
                    <tbody>
                    <tr v-for="roa in roasEmAberto" :key="roa.id">
                        <td>{{ roa.nome_id }}</td>
                        <td>{{ roa.intensidade }}</td>
                        <td>{{ roa.local }}</td>
                        <td>{{ roa.classificacao }}</td>
        ...
        <div class="row">
            <div class="col">
                <span><strong>Análise: </strong>{{ analise?.roas_em_aberto }}</span>
            </div>
        </div>
    </div>
</template>
