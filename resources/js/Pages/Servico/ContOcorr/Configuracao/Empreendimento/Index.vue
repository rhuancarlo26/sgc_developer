				...
				<Table
					...
					<template #body="{ item }">
						<tr>
							...
							<td>
								<button v-if="item.licenca?.geo_json" @click="abrirShapefile(item)" class="btn btn-icon btn-primary">
									<IconMap />
								</button>
								<span v-else>-</span>
							</td>
							<td>
								<NavButton v-if="!item.licenca?.geo_json" :icon="IconMap" class="btn-icon" type-button="success"
