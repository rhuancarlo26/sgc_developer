...
const trechos = computed(() => {
    let geojson = [];
    if (empreendimento.value.licenca?.geo_json) {
        geojson.push([empreendimento.value.licenca?.geo_json, modalShapefileMapa(empreendimento.value.licenca), empreendimento.value.licenca]);
    }
