...
const enviarLoteFiscal = () => {
    router.post(route('contratos.contratada.servicos.cont_ocorrencia.configuracao.lote_obra.enviar_lote_fiscal', {
        ...
        <Navbar :contrato="contrato" :servico="servico">
            <template #body>
                <ModelSearchFormAllColumns :columns="[]">
                    <template #action
                        v-if="!servico.cont_ocorr_parecer_configuracao || ![1, 3].includes(servico.cont_ocorr_parecer_configuracao.fk_status)">
                        <NavButton @click="enviarLoteFiscal()" type-button="primary" title="Enviar ao fiscal" />
                        <NavLink
                    ...
                    <template #body="{ item }">
                        <tr>
                            ...
                            <td class="d-inline-flex">
                                <NavButton @click="abrirModal(item)" type-button="info" class="btn-icon"
                                    :icon="IconEye" />

                                <template v-if="!servico.cont_ocorr_parecer_configuracao">
                                    <NavLink class="btn btn-icon btn-primary me-1"
                                        route-name="contratos.contratada.servicos.cont_ocorrencia.configuracao.lote_obra.create"
                                        :param="{ contrato: contrato.id, servico: servico.id, lote: item.id }"
                                        :icon="IconPencil" />
                                    <LinkConfirmation v-slot="confirmation"
                                        :options="{ text: 'A remoção do lote será permanente.' }">
