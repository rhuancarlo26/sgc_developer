...
const captureChart = () => {
    ...
    chart = document.getElementById('graf_reg_classe');

    html2canvas(chart, {
        useCORS: true,
        allowTaint: true
    }).then(canvas => {
        props.form.graf_reg_classe = canvas.toDataURL('image/png');
...
<template>
    <div name="graf_reg_classe" id="graf_reg_classe" class="d-flex justify-content-center mb-4">
        <PieChart :style="{ height: '400px', position: 'relative' }" :chart_data="classes"
                  :chart_options="chartOptions"/>
    </div>
    <div class="row mb-4">
        <div class="col">
            <InputLabel value="Análise" for="registros_classe"/>
            <textarea class="form-control" name="registros_classe" id="registros_classe" v-model="form.registros_classe"
                      rows="5"></textarea>
        </div>
    </div>
