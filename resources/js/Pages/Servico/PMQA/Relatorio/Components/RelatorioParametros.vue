...
const parametrosVinculados = computed(() => {
    return props.relatorio?.resultado?.campanhas.flatMap(campanha => campanha.pontos.flatMap(ponto => ponto.lista?.parametros).filter(parametro => parametro !== null && parametro !== undefined).reduce((acc, curr) => {
