...
const saveMedicao = () => {
	...
	if (form.id) {
		form.patch(route('contratos.contratada.servicos.pmqa.execucao.medir.update', {
		...
		form.post(route('contratos.contratada.servicos.pmqa.execucao.medir.store', {
			contrato: props.contrato.id,
			servico: props.servico.id,
			campanha: props.campanha.id,
			ponto: props.ponto.id
						...
						<table class="table card-table table-bordered">
							...
							<tbody>
								...
								<tr v-for="vinculado in ponto.ponto?.lista?.parametros_vinculados" :key="vinculado.id">
									...
									<td>{{ vinculado.parametro.limite ? vinculado.parametro.classe_2 : 'Sem limite' }}
									</td>
									<td>
										<input type="text" class="form-control" v-model="form.parametros[vinculado.parametro.id]">
									</td>
								</tr>
							</tbody>
						</table>
