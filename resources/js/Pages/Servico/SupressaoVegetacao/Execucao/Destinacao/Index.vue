...
<template>
    ...
    <AuthenticatedLayout>
                ...
                <ModelSearchFormAllColumns
                    ...
                    <template #action>
                        <a v-if="data.data?.length" class="btn btn-success me-1"
                            :href="route('contratos.contratada.servicos.supressao-vegetacao.execucao.destinacao.export', { servico: servico.id, _query: urlQueryParams })">
                            Exportar Excel
                        </a>
                        <button class="btn btn-success" type="button" @click="abrirModalCadastro()">
                            Cadastrar Destinação
                        </button>
                    </template>
                </ModelSearchFormAllColumns>
                ...
                <Table
                    ...
                    <template #body="{ item }">
                        <tr>
                            ...
                            <td class="text-center">{{item.pilhas?.map((pilha) => pilha.chave).join(', ') ?? '-'}}
                            </td>
        ...
        <ModalCadastro ref="modalCadastroRef" :pilhas="pilhas" :servico="servico" />
    </AuthenticatedLayout>
