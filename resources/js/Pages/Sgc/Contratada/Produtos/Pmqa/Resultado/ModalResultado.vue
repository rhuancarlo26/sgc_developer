...
watch(
    ...
    (item) => {
        ...
        if (item) {
            resposta = form.campanhas_selecionadas.find(
                (campanha) => campanha.id === item.id,
            )
                ? "Essa campanha já existe!"
                : null;
        } else {
            resposta = null;
        }
...
const adicionarCampanha = () => {
    const existe = form.campanhas_selecionadas.find(
        (campanha) => campanha.id === form.campanha.id,
    );
    if (!existe) {
...
const salvarResultado = () => {
    if (form.id) {
        form.patch(
            route("contratos.contratada.servicos.pmqa.resultado.update", {
        ...
        form.post(
            route("contratos.contratada.sgc.pmqa.resultado.store", {
                contrato: props.contrato.id,
            ...
            <div class="card-body">
                <div class="row mb-4">
                    ...
                    <div class="col form-group mb-4">
                        ...
                        <InputError
                            :message="form.errors.campanhas_selecionadas"
                        />
                        <div v-show="message">
                        ...
                        <table class="table card-table table-bordered">
                            ...
                            <tbody>
                                <tr
                                    v-for="(
                                        campanha, index
                                    ) in form.campanhas_selecionadas"
                                    :key="campanha.id"
                                >
