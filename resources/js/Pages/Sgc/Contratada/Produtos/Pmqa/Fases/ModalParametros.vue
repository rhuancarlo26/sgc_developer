...
import DivTabelaMedirIqa from "@/Pages/Servico/PMQA/Configuracao/Parametro/DivTabelaMedirIqa.vue";
...
const form = useForm({
    id: null,
    draftData_id: props.draftData.id,
    nome: null,
    medir_iqa: false,
    parametros: []
});
...
const reset = () => {
  form.id = null;
  form.draftData_id = props.draftData.id;
  form.nome = null;
...
const saveParametros = () => {
    console.log('Enviando campanhaId:', props.draftData.id); // Debug: mostra 7

    if (form.id) {
        form.patch(route('contratos.contratada.servicos.pmqa.configuracao.parametro.update', {
        ...
        // ✅ CORREÇÃO 2: APENAS campanhaId
        form.post(route('sgc.contratada.produtos.pmqa.criar_parametros', {
                                ...
                                <div>
                                    <div class="form-selectgroup form-selectgroup-boxes d-flex flex-column">
                                        <label class="form-selectgroup-item flex-fill">
                                            <input type="checkbox" id="parametroIqa" name="parametroIqa"
                                                   v-model="form.medir_iqa"
                                            ...
                                            <div class="form-selectgroup-label d-flex align-items-center p-3">
                                                ...
                                                <div class="form-selectgroup-label-content d-flex align-items-center">
                                                    <div>
                                                        <div class="font-weight-medium">Indice de qualidade das aguas
                                                            (IQA)
                                                        </div>
                                                        <div>
                                        ...
                                        <label v-for="parametro in parametros" :key="parametro.id"
                                               class="form-selectgroup-item flex-fill">
                                            <input type="checkbox" :id="'parametro-' + parametro.id"
                                                   :name="'parametro-' + parametro.id"
                                                   :value="parametro.id" v-model="form.parametros"
                                                   class="form-selectgroup-input">
                                            <div class="form-selectgroup-label d-flex align-items-center p-3">
                                                <div class="me-3">
                                                    <span class="form-selectgroup-check"></span>
                                                ...
                                                <div class="form-selectgroup-label-content d-flex align-items-center">
                                                    <div>
                                                        <div class="font-weight-medium">{{ parametro.parametro }} {{
                                                                parametro.unidade ? `- ${parametro.unidade}` : null
                                                            }}
                                                        </div>
                                                        <div>
                                                            <DivTabelaParametro v-show="isChecked(parametro.id)"
                                                                                :parametro="parametro"/>
                                                        </div>
                                                    </div>
                                                </div>
