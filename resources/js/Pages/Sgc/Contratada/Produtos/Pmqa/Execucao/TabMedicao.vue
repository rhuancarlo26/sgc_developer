	...
	() => {
		...
		if (props.ponto.medicao?.parametros.length) {
			props.ponto.medicao?.parametros.forEach(parametro => {
				medicaoParametro.value[parametro.parametro_id] = parametro.medicao;
			});
		}
	});
</script>
<template>
	<div v-if="ponto.medicao">
				...
				<table class="table table-hover non-hover">
					...
					<tbody>
						...
						<tr v-for="vinculado in ponto.ponto?.lista.parametros_vinculados" :key="vinculado.id">
							...
							<td>{{ medicaoParametro[vinculado.parametro_id] }}</td>
						</tr>
					</tbody>
				</table>
			</div>
						...
						<tr v-for="arquivo in ponto.medicao.arquivos" :key="arquivo.id">
							...
							<td>
								<a class="btn btn-icon btn-primary me-1" target="_blank"
									:href="route('contratos.contratada.sgc.pmqa.execucao.medir.show_arquivo', { contrato: props.contrato.id, pmqa: props.pmqa.id, campanha: props.campanha.id, arquivo: arquivo.id })">
									<IconEye />
								</a>
							</td>
						</tr>
		...
		<div v-else>
			<div class="row mb-4">
				<div class="col">
					<span><strong>Realizado a medição: </strong>{{ ponto.medicao?.medido ? 'Não' : 'Sim' }}</span>
				</div>
			</div>
			<div class="row">
				<div class="col">
					<span><strong>Justificativa: </strong>{{ ponto.medicao?.observacao }}</span>
				</div>
			</div>
		</div>
	</div>
