...
const pontosFiltrados = computed(() => {
    return props.pontos.filter(
        (ponto) => !ponto.length || arrayPontosSalvo.value.includes(ponto.id),
    );
});
...
const saveCampanha = () => {
    if (form.id) {
        form.patch(
            route("contratos.contratada.sgc.pmqa.execucao.update", {
