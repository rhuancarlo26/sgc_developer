...
const props = defineProps({
    ...
    produto: { type: String, default: 'Quilombola' },
...
const salvarRascunho = () => {
    console.log('Quilombola: salvarRascunho chamado', form);
    ...
    if (CardFotosRef.value?.validarCampos?.()) {
        console.log('Quilombola: validação de fotos falhou');
        return;
    }

    if (CardAnexosRef.value?.validarCampos?.()) {
        console.log('Quilombola: validação de anexos falhou');
        return;
    }
...
const enviarParaAnalise = () => {
    console.log('Quilombola: enviarParaAnalise chamado', form);
...
const submitForm = () => {
    const url = route('sgc.contratada.produtos.store', [props.contrato, produtoRoute]);
    console.log('Quilombola: submitForm', { url, form });
    ...
    form.post(url, {
        ...
        forceFormData: true,
        onSuccess: () => {
            console.log('Quilombola: form enviado com sucesso');
        ...
        onError: (errors) => {
            console.log('Quilombola: erros do post', errors);
        },
    });
};
        ...
        <template #header>
            <Breadcrumb
                :links="[
                    ...
                    { route: route('sgc.contratada.produtos.index', [props.contrato, produtoRoute]), label: props.contratos.contratada },
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <a v-if="form.modulo_id" :href="route('sgc.contratada.produtos.modulos.configuracoes.gerar-planilha-modelo', [props.contrato, produtoRoute, form.modulo_id])" class="btn btn-primary">Baixar</a>
