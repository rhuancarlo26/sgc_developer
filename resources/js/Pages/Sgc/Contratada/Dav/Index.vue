...
const progressValues = ref(
    props.subprodutos
        .filter((qtd) => {
            ...
            return (
                consumo.value[numeroFormatado] &&
                qtd.produto === "14" &&
                numeroFormatado === qtd.contrato
            );
        })
        .map((qtd) => {
            const numeroFormatado = props.contrato.numero_contrato.replace(
            ...
            const utilizada =
                consumo.value[numeroFormatado]?.[qtd.descricao_revisada] || 0;

            const percentage =
                total > 0 ? ((utilizada / total) * 100).toFixed(2) : 0;
...
const updatePages = (pages) => {
    for (const week of pages) {
        ...
        if (viewWeeks.length > 0) {
            const allDays = viewWeeks.flatMap(
                (viewWeek) => viewWeek.days || []
            );
            if (allDays.length > 0) {
                ...
                if (
                    newStart.getTime() !==
                        selectedRange.value.start.getTime() ||
            ...
            <div class="w-100 d-flex justify-content-between">
                ...
                <div>
                    <button
                        type="button"
                        class="btn btn-info"
                        @click="abrirVisualizarFormDav"
                    >
                        Cadastrar DAV
                    </button>
                    <a
                        class="btn btn-warning mx-3"
                        :href="'/sgc/gestao/dav/historicos/1'"
                        >DAV's Reprovadas</a
                    >
                </div>
            </div>
            ...
            <template #body>
                <div class="d-flex align-items-stretch">
                                        ...
                                        >
                                            ...
                                            <div>
                                                ...
                                                <small
                                                    ><strong> Fim:</strong>
                                                    {{
                                                        reminder.endDate.toLocaleDateString(
                                                            "pt-BR"
                                                        )
                                                    }}</small
                                                >
                    ...
                    <div class="card" style="flex: 1">
                        ...
                        <div class="p-5">
                            <div
                                v-for="(value, index) in progressValues"
                                :key="index"
                            >
                                <h4>{{ value.label }}</h4>
                                <div
                                ...
                                >
                                    ...
                                    <p class="ms-3" style="margin-top: 5px">
                                        Total: {{ value.total }}
                                    </p>
                                </div>
                            </div>
