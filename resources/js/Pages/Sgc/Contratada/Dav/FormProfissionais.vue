...
const submitForm = async () => {
    ...
    try {
        ...
        await form.post(route("sgc.gestao.storeDavProfissionais"), {
            ...
            onSuccess: () => {
                console.log(
                    "Todos os profissionais foram enviados com sucesso!"
                );
            },
            onError: (errors) => {
...
onMounted(() => {
    const modal = modalVisualizarForm.value.getBsModal();
    const el = modal._element;

    el.addEventListener("hidden.bs.modal", () => {
        document.body.classList.remove("modal-profissionais-open");
    });
});
...
defineExpose({
    abrirModal: () => {
        document.body.classList.add("modal-profissionais-open");
...
<template>
    <Modal
        ...
        modal-dialog-class="modal-xl modal-profissionais"
    >
        <template #body>
            <form
                class="d-flex align-items-end gap-3"
                @submit.prevent="adicionarProfissional"
            >
                <div class="w-100">
                ...
                <div class="w-100">
                    <label for="inputFormacao" class="form-label"
                        >Formação</label
                    >
                    <input
                        type="text"
