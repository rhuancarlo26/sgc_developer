...
const somaExtensao = computed(() => {
    return (props.licenca?.segmentos ?? [])

        .map(s => Number(s.extensao_br) || 0)

        .reduce((a, b) => a + b, 0);
});

const somaExtensaoDisplay = computed(() => {
    const segs = props.licenca?.segmentos ?? [];

    if (segs.length === 0) {
        return 'Sem segmentos cadastrados';
    }

    return somaExtensao.value;
});
...
<template>
    <div class="row mb-4">
        <div class="col">
            <InputLabel value="Tipo de licença:" for="tipo_rel" />
    ...
    <div class="row mb-4">
        <div class="col">
            <InputLabel value="Extensão:" for="soma_extensao" />

            <!-- valor de só-leitura, vinculando ao computed display -->
            <input type="text" id="soma_extensao" name="soma_extensao" class="form-control" :value="somaExtensaoDisplay"
                disabled readonly />
