...
const form = useForm({
	name: null,
	...props.role,
	permissions: props.role.permissions ? props.role.permissions.map(a => a.name) : []
})

const saveRole = () => {

	form.transform((data) => Object.assign({ id: props.role.id }, data))

	if (props.role.id) {
		form.patch(route('cadastros.perfis.atualizar', props.role.id));
		return;
	}

	form.post(route('cadastros.perfis.criar'));
}
...
const handlePrefixRowWheel = (event) => {
		...
		<form @submit.prevent="saveRole">
				...
				<div class="card-body bg-gray-500 space-y-4">
					...
					<template v-for="section in getRoutesSections()">
						...
						<div class="card">
							...
							<div class="card-body">
								<div class="d-flex align-items-top prefix-row" @wheel="handlePrefixRowWheel">
									...
									<template v-for="prefix in getPrefixesBySection(section)">
										<div class="col prefix-col mx-2">
											<table class="table table-striped table-bordered">
												<thead v-show="withoutSpecialChars(prefix) !== withoutSpecialChars(section)">
													<tr class="table-secondary">
														<td colspan="2" role="button" title="Selecionar tudo" @click="togglePrefixRoutes(prefix)">
															<h4 class="my-0">
																{{ prefix }}
															</h4>
														</td>
												...
												<tbody>
													<tr v-for="(route) in getRoutesByPrefix(prefix)" :key="`${route.alias}`">
														<td>
															<label class="form-check-label" :for="`checkRoute${route.alias}`">
																{{ getRouteLabel(route.alias) }}
															</label>
														</td>
														<td class="w-1 text-center">
															<input name="permissions" v-model="form.permissions" class="form-check-input"
																type="checkbox" :value="route.alias" :id="`checkRoute${route.alias}`" />
														</td>
													</tr>
												</tbody>
											</table>
			...
			<div class="card card-body">
				<div class="d-flex justify-content-end gap-2">
					...
					<LinkConfirmation v-if="role.id" v-slot="confirmation"
						:options="{ text: 'A remoção de um perfil afetará as permissões de todos os usuaŕios associados a ele' }">
						<Link :onBefore="confirmation.show" :href="route('cadastros.perfis.deletar', role.id)" as="button"
							method="delete" type="button" class="btn btn-danger">
