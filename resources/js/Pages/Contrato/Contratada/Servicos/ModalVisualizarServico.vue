...
const temHistoricoRetorno = computed(() => {
    return (detalhe.value?.retornos_confeccao ?? []).length > 0;
});
...
const abrirHistoricoRetorno = () => {
    ...
    setTimeout(() => {
        ModalHistoricoRetornoRef.value?.abrirModal?.(detalhe.value);
    }, 150);
};
...
<template>
    <Modal ref="modalDetalhes" title="Visualizar serviço cadastrado" modal-dialog-class="modal-xl">
        <template #body>
            <div v-if="temHistoricoRetorno"
                class="alert alert-warning d-flex justify-content-between align-items-center">
                <div>
                    Este serviço retornou para em confecção.
                </div>

                <button type="button" class="btn btn-sm btn-warning" @click="abrirHistoricoRetorno">
                    Ver histórico de retorno
                </button>
            </div>
            <div class="mb-4">
