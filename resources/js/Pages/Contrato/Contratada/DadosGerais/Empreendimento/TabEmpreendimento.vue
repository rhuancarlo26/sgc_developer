...
watch(
	...
	(UF) => {
		...
		if (UF) {
			...
			uf_rodovias.value = props.rodovias.filter(r => r.estados_id === UF.id)
		} else {
...
<template>
	...
	<div class="row mb-4 mt-4">
		...
		<div class="col">
			...
			<v-select id="uf" :options="ufs" label="uf" v-model="form_trecho.uf" placeholder="Selecione uma UF">
				<template #no-options>
		...
		<template v-if="can('contratos.contratada.store_empreendimento_trecho')">
			<div class="col">
				<InputLabel value="Rodovia" for="rodovia" />
				<v-select id="rodovia" :options="uf_rodovias" label="rodovia" v-model="form_trecho.rodovia"
					:disabled="!uf_rodovias.length" placeholder="Primeiro selecione a UF">
					<template #no-options>
						Nenhum registro encontrado.
					</template>
				</v-select>
						...
						<span class="dropdown">
							...
							<div class="dropdown-menu dropdown-menu-end" style="">
								<a @click="Object.assign(form_trecho, trecho)" class="dropdown-item" href="javascript:void(0)"
									v-if="can('contratos.contratada.update_empreendimento_trecho')">
									Editar
								</a>
								<a @click="excluirTrecho(trecho)" class="dropdown-item" href="javascript:void(0)"
									v-if="can('contratos.contratada.destroy_empreendimento_trecho')">
									Excluir
								</a>
							</div>
						</span>
