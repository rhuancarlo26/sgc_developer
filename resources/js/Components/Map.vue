        ...
        try {
            ...
            linestring_array.forEach(geojson_linestring => {
                if (popupAndEvent) { // Define coordenadas e dados do evento
                    let geojson = typeof geojson_linestring[0] === 'string'
                        ? JSON.parse(geojson_linestring[0])
                        : geojson_linestring[0];
                    ...
                    const geojson = typeof geojson_linestring === 'string'
                        ? JSON.parse(geojson_linestring)
                        : geojson_linestring;
...
const zoomToLinestring = (geojson_linestring) => {
    try {
        ...
        if (geojson.type === 'LineString') {
            ...
            latLngArrat = geojson.coordinates[0].map(coord => [coord[1], coord[0]]);
        } else if (geojson.type === 'MultiPolygon') {
            geojson.coordinates.forEach(polygon => {
                polygon[0].forEach(coord => {
                    latLngArrat.push([coord[1], coord[0]]);
                });
            });
        }

        map.fitBounds(L.latLngBounds(latLngArrat));
