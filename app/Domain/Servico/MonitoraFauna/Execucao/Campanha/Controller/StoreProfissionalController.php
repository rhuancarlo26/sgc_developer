...
class StoreProfissionalController extends Controller
{
  ...
  {
    $response = $this->campanhaService->store_profissional(post: $request->all());

    return to_route('contratos.contratada.servicos.monitora_fauna.execucao.campanha.create', ['contrato' => $contrato->id, 'servico' => $servico->id, 'campanha' => $request->id_campanha])->with('message', $response['request']);
  }
}
