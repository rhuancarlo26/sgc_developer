...
class DestroyDocumentoEquipamentoRecursoController extends Controller
{
  ...
  public function index(Contrato $contrato, RecursoEquipamentoDocumento $documento)
  {
    try {
      ...
      return to_route('contratos.contratada.recurso.equipamento.create', ['contrato' => $contrato->id, 'equipamento' => $documento->equipamento_id])->with('message', ['type' => 'success', 'content' => 'Documento excluido com sucesso!']);
    } catch (\Exception $e) {
      return to_route('contratos.contratada.recurso.equipamento.create', ['contrato' => $contrato->id, 'equipamento' => $documento->equipamento_id])->with('message', ['type' => 'error', 'content' => $e->getMessage()]);
    }
  }
}