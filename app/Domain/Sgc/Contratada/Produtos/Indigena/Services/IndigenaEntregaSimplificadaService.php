...
namespace App\Domain\Sgc\Contratada\Produtos\Indigena\Services;
...
class IndigenaEntregaSimplificadaService
{
    public function criar(array $dados): SgcIndigenaCampanha
    {
        return DB::transaction(function () use ($dados) {
            $status = $dados['enviar_analise'] ? 'Em análise' : 'Em elaboração';
            $campanha = SgcIndigenaCampanha::create([
            ...
            $entrega = SgcEntregaSimplificada::create([
                'id_contrato' => $campanha->id_contrato, 'produto_tipo' => 'indigena',
                'entidade_tipo' => 'indigena_campanha', 'entidade_id' => $campanha->id,
    ...
    public function atualizar(SgcIndigenaCampanha $campanha, SgcEntregaSimplificada $entrega, array $dados): void
    {
    ...
    public function registrarAnalise(SgcIndigenaCampanha $campanha, SgcEntregaSimplificada $entrega, string $status, ?string $observacoes, int $fiscalId): void
    {
        DB::transaction(function () use ($campanha, $entrega, $status, $observacoes, $fiscalId) {
            $campanha = SgcIndigenaCampanha::lockForUpdate()->findOrFail($campanha->id);
    ...
    private function guardarPlanilha(?UploadedFile $arquivo, int $campanhaId): array
    {
        ...
        $caminho = $arquivo->storeAs("entrega_simplificada/indigena/{$campanhaId}/planilhas", uniqid().'_'.$nome, 'public');
        ...
        foreach (['fotos' => 'foto', 'anexos' => 'anexo'] as $campo => $tipo) {
            foreach ($dados[$campo] ?? [] as $item) {
                if (!empty($item['arquivo']) && $item['arquivo'] instanceof UploadedFile) {
                    ...
                    $caminho = $arquivo->storeAs("entrega_simplificada/indigena/{$entrega->entidade_id}/{$campo}", uniqid().'_'.$nome, 'public');
                    $this->sincronizarArquivoPublico($caminho);
