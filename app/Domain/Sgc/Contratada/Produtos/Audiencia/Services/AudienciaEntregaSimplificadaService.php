...
namespace App\Domain\Sgc\Contratada\Produtos\Audiencia\Services;
...
class AudienciaEntregaSimplificadaService
{
    public function criar(array $dados): SgcAudienciaCampanha
    {
        return DB::transaction(function () use ($dados) {
            $status = $dados['enviar_analise'] ? 'Em análise' : 'Em elaboração';
            $campanha = SgcAudienciaCampanha::create([
            ...
            $entrega = SgcEntregaSimplificada::create([
                'id_contrato' => $campanha->id_contrato, 'produto_tipo' => 'audiencia',
                'entidade_tipo' => 'audiencia_campanha', 'entidade_id' => $campanha->id,
    ...
    public function atualizar(SgcAudienciaCampanha $campanha, SgcEntregaSimplificada $entrega, array $dados): void
    {
    ...
    public function registrarAnalise(SgcAudienciaCampanha $campanha, SgcEntregaSimplificada $entrega, string $status, ?string $observacoes, int $fiscalId): void
    {
        DB::transaction(function () use ($campanha, $entrega, $status, $observacoes, $fiscalId) {
            $campanha = SgcAudienciaCampanha::lockForUpdate()->findOrFail($campanha->id);
    ...
    private function guardarPlanilha(?UploadedFile $arquivo, int $campanhaId): array
    {
        ...
        $caminho = $arquivo->storeAs("entrega_simplificada/audiencia/{$campanhaId}/planilhas", uniqid().'_'.$nome, 'public');
        ...
        foreach (['fotos' => 'foto', 'anexos' => 'anexo'] as $campo => $tipo) {
            foreach ($dados[$campo] ?? [] as $item) {
                if (!empty($item['arquivo']) && $item['arquivo'] instanceof UploadedFile) {
                    ...
                    $caminho = $arquivo->storeAs("entrega_simplificada/audiencia/{$entrega->entidade_id}/{$campo}", uniqid().'_'.$nome, 'public');
                    $this->sincronizarArquivoPublico($caminho);
