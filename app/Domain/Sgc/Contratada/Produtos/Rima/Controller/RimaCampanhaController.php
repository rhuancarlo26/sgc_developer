...
namespace App\Domain\Sgc\Contratada\Produtos\Rima\Controller;

use App\Shared\Http\Controllers\Controller;
use App\Domain\Sgc\Contratada\Produtos\Rima\Services\RimaFiscalService;
use App\Domain\Sgc\Contratada\Produtos\Rima\Requests\RimaAnaliseRequest;
...
class RimaCampanhaController extends Controller
{
    public function __construct(
        protected RimaFiscalService $rimaFiscalService,
    ) {}

    public function show($contrato, $produto, $campanhaId)
    {
        $campanha = SgcRima::with(['modulo', 'fotos', 'anexos', 'analises.fiscal'])
        ...
        return Inertia::render('Sgc/Contratada/Produtos/Rima/VisualizarCampanha', [
    ...
    public function analise($contrato, $produto, $campanha)
    {
        if (Auth::user()->perfis_id !== 3) {
            return redirect()
                ->route('sgc.contratada.produtos.rima.show', [$contrato, $produto, $campanha])
                ->withErrors(['error' => 'Acesso negado. Apenas fiscais podem analisar campanhas.']);
        }

        $campanhaObj = SgcRima::with(['modulo', 'fotos', 'anexos'])
            ->findOrFail($campanha);
        ...
        return Inertia::render('Sgc/Contratada/Produtos/Rima/AnaliseCampanha', [
            'campanha' => $campanhaData,
            'contrato' => $campanhaObj->id_contrato,
            'produto' => 'rima',
            'contratos' => ['contratada' => 'Nome da Contratada', 'tipo_contrato' => 'Tipo'],
            'canApprove' => Auth::user()->perfis_id === 3 && $campanhaObj->status === 'Em análise',
            'analises' => $this->rimaFiscalService->getAnalisesByCampanha($contrato, $campanha) ?? [],
        ]);
    }

    public function salvarAnalise(RimaAnaliseRequest $request, $contrato, $produto, $campanha)
    {
        if (Auth::user()->perfis_id !== 3) {
        ...
        try {
            ...
            $this->rimaFiscalService->salvarAnalise($contrato, $campanha, $validated);

            return redirect()
                ->route('sgc.contratada.produtos.rima.analise', [$contrato, $produto, $campanha])
                ->with('success', 'Análise salva com sucesso!');

        } catch (\Exception $e) {
            Log::error('RimaCampanhaController@salvarAnalise: erro', [
    ...
    public function finalizarAvaliacao($contrato, $produto, $campanha)
    {
        ...
        try {
            $this->rimaFiscalService->finalizarAvaliacaoCampanha($contrato, $campanha);

            return redirect()
                ->route('sgc.contratada.produtos.index', [$contrato, 'rima'])
                ->with('success', 'Campanha avaliada com sucesso!');

        } catch (\Exception $e) {
            Log::error('RimaCampanhaController@finalizarAvaliacao: erro', [
                'contrato_id' => $contrato,
                'campanha_id' => $campanha,
                'erro' => $e->getMessage(),
            ]);
    ...
    public function aprovarTudo($contrato, $produto, $campanha)
    {
        ...
        try {
            $campanhaObj = SgcRima::findOrFail($campanha);
            ...
            $this->rimaFiscalService->salvarAnalise($contrato, $campanha, [
                'status' => 'Aprovada',
                'observacoes' => null,
            ]);
            ...
            Log::error('RimaCampanhaController@aprovarTudo: erro', [
                'contrato_id' => $contrato,
                'campanha_id' => $campanha,
                'erro' => $e->getMessage(),
            ]);
    ...
    public function reprovarTudo(Request $request, $contrato, $produto, $campanha)
    {
        ...
        try {
            ...
            Log::error('RimaCampanhaController@reprovarTudo: erro', [
                'contrato_id' => $contrato,
                'campanha_id' => $campanha,
                'erro' => $e->getMessage(),
            ]);
    ...
    {
        $campanha = SgcRima::with(['modulo', 'fotos', 'anexos'])
            ->where('id_contrato', $contrato)
            ->findOrFail($campanhaId);

        if (!in_array($campanha->status, ['Rejeitada', 'Em elaboração'])) {
            return redirect()
                ->route('sgc.contratada.produtos.rima.show', [$contrato, $produto, $campanhaId])
        ...
        return Inertia::render('Sgc/Contratada/Produtos/Rima/EditCampanha', [
            'campanha' => [
                'id' => $campanha->id,
                'cod_emp' => $campanha->cod_emp,
                'id_campanha' => $campanha->id_campanha,
    ...
    public function update($contrato, $produto, $campanhaId)
    {
        ...
        $campanha = SgcRima::findOrFail($campanhaId);
        ...
        try {
            ...
            if (isset($validated['arquivo'])) {
                ...
                $caminho = $arquivo->storeAs('Rima/Planilhas', $nomeUnico, 'public');
            ...
            DB::rollBack();
            Log::error('RimaCampanhaController@update: erro', [
                'contrato_id' => $contrato,
                'campanha_id' => $campanhaId,
                'erro' => $e->getMessage(),
            ]);
