...
use App\Models\SgcRimaFoto;
use App\Models\SgcRimaAnexo;
use Illuminate\Http\UploadedFile;
use Illuminate\Support\Facades\Storage;

class RimaService
{
    public function store(array $data): SgcRima
    {
        ...
        $rima = SgcRima::create($data);

        try {
            $this->gerenciarFotos($rima, $fotos);
            $this->gerenciarAnexos($rima, $anexos);
    ...
    private function gerenciarFotos(SgcRima $rima, array $fotos): void
    {
        $idsFotos = [];
        ...
        foreach ($fotos as $f) {
            ...
            if (!empty($f['arquivo']) && $f['arquivo'] instanceof UploadedFile) {
                ...
                $caminho = $arquivoF->storeAs('Rima/Fotos', $nomeUnico, 'public');
                $this->sincronizarArquivoPublico($caminho);
            ...
            $foto = SgcRimaFoto::updateOrCreate(
                ['id' => $f['id'] ?? null],
                $dadosFoto
            );
    ...
    private function gerenciarAnexos(SgcRima $rima, array $anexos): void
    {
        ...
        foreach ($anexos as $a) {
            ...
            if (!empty($a['arquivo']) && $a['arquivo'] instanceof UploadedFile) {
                ...
                $caminho = $arquivoA->storeAs('Rima/Anexos', $nomeUnico, 'public');
                $this->sincronizarArquivoPublico($caminho);
            ...
            $anexo = SgcRimaAnexo::updateOrCreate(
                ['id' => $a['id'] ?? null],
                [
                    'sgc_rima_id' => $rima->id,
                    ...$dataA,
