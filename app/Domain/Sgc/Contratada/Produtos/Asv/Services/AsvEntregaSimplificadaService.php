...
namespace App\Domain\Sgc\Contratada\Produtos\Asv\Services;
...
class AsvEntregaSimplificadaService
{
    public function criar(array $dados): SgcAsvCampanha
    {
        return DB::transaction(function () use ($dados) {
            $status = $dados['enviar_analise'] ? 'Em análise' : 'Em elaboração';
            $campanha = SgcAsvCampanha::create([
            ...
            $entrega = SgcEntregaSimplificada::create([
                'id_contrato' => $campanha->id_contrato, 'produto_tipo' => 'asv',
                'entidade_tipo' => 'asv_campanha', 'entidade_id' => $campanha->id,
    ...
    public function atualizar(SgcAsvCampanha $campanha, SgcEntregaSimplificada $entrega, array $dados): void
    {
    ...
    public function registrarAnalise(SgcAsvCampanha $campanha, SgcEntregaSimplificada $entrega, string $status, ?string $observacoes, int $fiscalId): void
    {
        DB::transaction(function () use ($campanha, $entrega, $status, $observacoes, $fiscalId) {
            $campanha = SgcAsvCampanha::lockForUpdate()->findOrFail($campanha->id);
    ...
    private function guardarPlanilha(?UploadedFile $arquivo, int $campanhaId): array
    {
        ...
        $caminho = $arquivo->storeAs("entrega_simplificada/asv/{$campanhaId}/planilhas", uniqid().'_'.$nome, 'public');
        ...
        foreach (['fotos' => 'foto', 'anexos' => 'anexo'] as $campo => $tipo) {
            foreach ($dados[$campo] ?? [] as $item) {
                if (!empty($item['arquivo']) && $item['arquivo'] instanceof UploadedFile) {
                    ...
                    $caminho = $arquivo->storeAs("entrega_simplificada/asv/{$entrega->entidade_id}/{$campo}", uniqid().'_'.$nome, 'public');
                    $this->sincronizarArquivoPublico($caminho);
