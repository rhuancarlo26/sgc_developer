...
namespace App\Domain\Sgc\Contratada\Produtos\Quilombola\Services;
...
class QuilombolaEntregaSimplificadaService
{
    public function criar(array $dados): SgcQuilombolaCampanha
    {
        return DB::transaction(function () use ($dados) {
            $status = $dados['enviar_analise'] ? 'Em análise' : 'Em elaboração';
            $campanha = SgcQuilombolaCampanha::create([
            ...
            $entrega = SgcEntregaSimplificada::create([
                'id_contrato' => $campanha->id_contrato, 'produto_tipo' => 'quilombola',
                'entidade_tipo' => 'quilombola_campanha', 'entidade_id' => $campanha->id,
    ...
    public function atualizar(SgcQuilombolaCampanha $campanha, SgcEntregaSimplificada $entrega, array $dados): void
    {
    ...
    public function registrarAnalise(SgcQuilombolaCampanha $campanha, SgcEntregaSimplificada $entrega, string $status, ?string $observacoes, int $fiscalId): void
    {
        DB::transaction(function () use ($campanha, $entrega, $status, $observacoes, $fiscalId) {
            $campanha = SgcQuilombolaCampanha::lockForUpdate()->findOrFail($campanha->id);
    ...
    private function guardarPlanilha(?UploadedFile $arquivo, int $campanhaId): array
    {
        ...
        $caminho = $arquivo->storeAs("entrega_simplificada/quilombola/{$campanhaId}/planilhas", uniqid().'_'.$nome, 'public');
        ...
        foreach (['fotos' => 'foto', 'anexos' => 'anexo'] as $campo => $tipo) {
            foreach ($dados[$campo] ?? [] as $item) {
                if (!empty($item['arquivo']) && $item['arquivo'] instanceof UploadedFile) {
                    ...
                    $caminho = $arquivo->storeAs("entrega_simplificada/quilombola/{$entrega->entidade_id}/{$campo}", uniqid().'_'.$nome, 'public');
                    $this->sincronizarArquivoPublico($caminho);
